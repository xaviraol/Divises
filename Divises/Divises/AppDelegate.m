...
//  Divises
//
//  Created by Xavier Ramos on 23/2/16.
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"activeCurrencies"] == nil) {
        ...
        for (NSString *key in [currenciesDictionary allKeys]) {
            if ([key isEqualToString:@"EUR"]|| [key isEqualToString:@"USD"]|| [key isEqualToString:@"GBP"]||[key isEqualToString:@"CNY"] ) {
                NSDictionary *active = [currenciesDictionary objectForKey:key];
                if ([key isEqualToString:@"USD"]) {
                    [[NSUserDefaults standardUserDefaults]setValue:active forKey:@"mainCurrency"];
                }
                [defaultCurrencies addObject:active];
                [[NSUserDefaults standardUserDefaults] setValue:defaultCurrencies forKey:@"activeCurrencies"];
            }
        }
    }
